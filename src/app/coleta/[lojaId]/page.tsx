      ...
      <main className="flex-1 p-4 max-w-lg w-full mx-auto pb-24">
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-3">
              <label className="text-xs font-bold text-slate-300 flex items-center gap-2">
                <User className="w-4 h-4 text-cyan-400" /> Quem é você? (Motoboy)
              </label>
              ...
              {motoboys.length > 0 ? (
                <select
                  ...
                  onChange={(e) => {
                    const sel = motoboys.find((m) => m.id === e.target.value);
                    setMotoboySelecionadoId(e.target.value);
                    setMotoboyNome(sel?.nome || '');
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-3.5 text-sm font-bold text-white outline-none focus:border-cyan-500"
                >
                  {motoboys.map((m) => (
                    <option key={m.id} value={m.id}>
                      {m.nome} ({m.veiculo || 'Moto'})
                    </option>
                  ))}
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  >
                    {MODELOS_IPHONE_ORDENADOS.map((m) => (
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-2.5 text-xs text-white outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-2.5 text-xs font-mono text-cyan-300 outline-none"
                  />
                </div>
              </div>
            </div>
        ...
        {etapa === 5 && (
          <div className="space-y-4 animate-in fade-in duration-200">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-2">
              <label className="text-xs font-bold text-slate-300 block">
                Observações do Motoboy
              </label>
              <textarea
                rows={2}
                placeholder="Ex: Cliente entregou com capinha e cabo original. Sem fonte."
